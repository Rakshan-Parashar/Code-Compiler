...
if (typeof window !== 'undefined' && !window.api) {
  ...
  async function callBackend(endpoint, method = 'GET', body = null, useToken = true) {
    ...
    if (useToken) {
      const accStr = localStorage.getItem('zenith_account');
      if (accStr) {
        try {
  ...
  const defaultFs = {
    ...
    tree: [
      ...
      },
      {
        ...
        type: 'file',
        ext: 'js',
        content: 'console.log("Welcome to Zenith IDE!");\n\n// Press Run to simulate execution.',
        size: 73,
        mtime: Date.now()
      },
      {
  ...
  const getVirtualFs = () => {
    const data = localStorage.getItem('zenith_virtual_fs');
    if (data) {
      try {
  ...
  const saveVirtualFs = (fsData) => {
    localStorage.setItem('zenith_virtual_fs', JSON.stringify(fsData));
  };
  ...
  window.api = {
    ...
    // Settings
    loadSettings: async () => {
      const s = localStorage.getItem('zenith_settings');
      return s ? JSON.parse(s) : null;
    },
    saveSettings: async (s) => {
      localStorage.setItem('zenith_settings', JSON.stringify(s));
      return { ok: true };
    },

    // Account
    loadAccount: async () => {
      const acc = localStorage.getItem('zenith_account');
      return acc ? JSON.parse(acc) : null;
    },
    saveAccount: async (a) => {
      localStorage.setItem('zenith_account', JSON.stringify(a));
      return { ok: true };
    },
    logout: async () => {
      localStorage.removeItem('zenith_account');
      return { ok: true };
    },
    accountSignup: async (name, email, password) => {
      ...
      if (res.ok && res.token) {
        ...
        localStorage.setItem('zenith_account', JSON.stringify(accountData));
    ...
    cloudList: async () => {
      ...
      const localCloud = localStorage.getItem('zenith_cloud_fallback');
    ...
    cloudSave: async (s) => {
      ...
      localStorage.setItem('zenith_cloud_fallback', JSON.stringify(list));
      return { ok: true, list };
    },
    ...
    // Recent + bookmarks
    loadRecent: async () => {
      const r = localStorage.getItem('zenith_recent');
      return r ? JSON.parse(r) : [];
    },
    loadBookmarks: async () => {
      const b = localStorage.getItem('zenith_bookmarks');
      return b ? JSON.parse(b) : [];
    },
    saveBookmarks: async (b) => {
      localStorage.setItem('zenith_bookmarks', JSON.stringify(b));
      return { ok: true };
    },
    ...
    runCode: async (code, lang) => {
      ...
      setTimeout(() => {
        if (['js', 'javascript'].includes(lang)) {
          ...
          trigger('exec:out', `Zenith Python Runner (Web Shim):\r\n`);
    ...
    ptyCreate: async (cwd) => {
      setTimeout(() => {
        trigger('pty:data', `Zenith Web PTY Terminal Initialized.\r\n$ `);
      }, 100);
      return { ok: true };
    },
    ptyWrite: (d) => {
    ...
    notebooksList: async (root) => {
      const data = localStorage.getItem('zenith_notebooks');
      return { ok: true, notebooks: data ? JSON.parse(data) : [] };
    },
    notebooksCreate: async (root, name) => {
      const current = localStorage.getItem('zenith_notebooks');
      const list = current ? JSON.parse(current) : [];
      if (!list.includes(name)) {
        list.push(name);
        localStorage.setItem('zenith_notebooks', JSON.stringify(list));
      }
      localStorage.setItem(`zenith_notebook_${name}`, JSON.stringify({ cells: [] }));
      return { ok: true, name };
    },
    notebooksRead: async (root, name) => {
      const cellData = localStorage.getItem(`zenith_notebook_${name}`);
      return { ok: true, content: cellData ? JSON.parse(cellData) : { cells: [] } };
    },
    notebooksWrite: async (root, name, content) => {
      localStorage.setItem(`zenith_notebook_${name}`, JSON.stringify(content));
      return { ok: true };
    },
    notebooksDelete: async (root, name) => {
      ...
      localStorage.setItem('zenith_notebooks', JSON.stringify(list));
      localStorage.removeItem(`zenith_notebook_${name}`);
      return { ok: true };
    }
  };
}
