...
ipcMain.handle('lint:js', (_, data) => {
  ...
  if (ext === 'json') {
    if (code.trim() !== '') {
      try {
        ...
        const match = /position\s+(\d+)/.exec(e.message);
        if (match) {
        ...
        errors.push({ line: lineNum, col: colNum, msg: e.message, sev: 'error' });
      }
    }
  ...
  code.split('\n').forEach((line, i) => {
    ...
    if (['c', 'cpp', 'h'].includes(ext)) {
      if (trimmed === 'ret' || trimmed.startsWith('ret ') || trimmed.startsWith('ret;')) {
        errors.push({
          line: i + 1,
          col: line.indexOf('ret') + 1,
          msg: "Unknown symbol 'ret'. Did you mean 'return'?",
          sev: 'error'
        });
      }
      
      const isStatement = /^(int|char|float|double|bool|void|double|long|short|unsigned|signed|struct|class|return|printf|scanf|cin|cout|\w+\s*\(|\w+\s*=)/.test(trimmed);
      const endsWithSeparator = /[;{}]$|^\/\/|^\/\*|^#/.test(trimmed) || trimmed === '';
      if (isStatement && !endsWithSeparator) {
        errors.push({
          line: i + 1,
          col: line.length + 1,
    ...
    if (ext === 'py') {
      const needsColon = /^(if|elif|else|for|while|def|class)\b/.test(trimmed);
      const endsWithColon = /:\s*(#.*)?$/.test(trimmed) || /^\s*#/.test(trimmed) || trimmed === '';
      if (needsColon && !endsWithColon) {
        errors.push({
          line: i + 1,
          col: line.length + 1,
          msg: "Colon expected ':'",
      ...
      if (/^\s*print\s+['"]/.test(line)) {
        errors.push({
          line: i + 1,
          col: line.indexOf('print') + 1,
          msg: "Missing parentheses in call to 'print'. Did you mean print(...)?",
          sev: 'error'
        });
      }
    }
    ...
    if (ext === 'css') {
      const isCssProperty = /^[a-zA-Z\-]+\s*:/.test(trimmed);
      if (isCssProperty && !trimmed.endsWith(';') && !trimmed.endsWith('}')) {
        errors.push({
          line: i + 1,
          col: line.length + 1,
          msg: "Semicolon expected ';'",
...
const getNotebooksDir = (root) => {
  ...
  return path.join(root, '.atmos', 'notebooks');
};
